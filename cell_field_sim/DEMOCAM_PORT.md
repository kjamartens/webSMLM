...
Cytoplasm: rim height 0.1-0.3, edge rise 0.1-0.5, mid height 1-2, mid distance 0.1-0.3 x cell radius, `nucMargin` 0.6. `cytoMaxSlope` (default 0.5 µm/µm, 0 = off, no slider) caps how fast the cytoplasm may rise outside the nucleus-margin band: the edge rise is `Hc(1-exp(-s·dEdge/Hc))` with `Hc = 2·cytoMidHeight` (saturating, so no linear pyramid), the rim→mid ramp is stretched to keep its peak slope ≤ s, and `envelopNucleus(c, margin, cytoSlopeRunout)` pushes the outline out so the nucleus is always ≥ `nucMargin + 2·mid·ln2/s` from the edge. The four `cyto*` per-cell fields are now sampled before `envelopNucleus`. `cytoSmoothPasses` (default 12, slider 0-12) is the number of 3x3 binomial smoothing passes `smoothCytoGrid` applies to the mesh; `cytoRings` (default 60, slider 4-60) is the mesh's radial ring count; `cytoTheta` (default 128, slider 32-128) is its angular sample count.
